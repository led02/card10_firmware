...
{
  hciReadLocalVerInfo_t evt;

  LlGetVersion(&evt.manufacturerName, &evt.hciVersion, &evt.hciRevision);

  evt.hdr.param = 0;
  evt.hdr.event = HCI_READ_LOCAL_VER_INFO_CMPL_CBACK_EVT;
  evt.hdr.status = HCI_SUCCESS;
  evt.status = HCI_SUCCESS;
  evt.lmpVersion = evt.hciVersion;     /* LMP_Version */
  evt.lmpSubversion = evt.hciRevision; /* LMP_Subversion */
